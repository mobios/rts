...
void graphics::objectLoader::load(const char *objPath, const char *texPath){
	...
	objFile.open(objPath, std::fstream::in);
	...
	engine::renderEngine::registerModel(new model(loadBMP(texPath), &outVertices, &outUVs, &outNorms));
}

GLuint graphics::objectLoader::loadBMP(const char *path){
	std::fstream texFile;
	texFile.open(path, std::fstream::in);
